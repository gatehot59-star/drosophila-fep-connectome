...
**Última actualización:** 2026-08-24 23:15 (America/Buenos_Aires) · **Se sobreescribe, no se acumula.**
...
| **Bates, A. S., Phelps, J. S., Kim, M., Yang, H. H., et al. (2026).** *Distributed control circuits across a brain-and-cord connectome.* Nature | doi:10.1038/s41586-026-10735-w | **cerebro + cordón**, publicado **8-jun-2026**, preprint **31-jul-2025** | métrica de **influence** lineal, `R² = 0,94` en 94.278 pares |
...
**Su Table 2, verbatim, y hay que tenerla a mano:** reciprocidad 0,138 = **×858 vs ER, ×43,8 vs CFG, ×45,9 vs NND, ×7,22 vs NPC**. Clustering 0,0463 = ×144 / ×7,57 / ×10,9 / ×2,88.
...
**Este es el hallazgo más caro del día, porque tumba el pivote tal como estaba escrito.** Verbatim de Lin:
...
**Está en el CUERPO del paper, no en un suplementario.** Y no es solo reciprocidad por región: **es reciprocidad sobre densidad por región**, que es exactamente la forma del claim del abstract del Paper 1.

**Lo que SÍ sobrevive, y es un claim más chico pero verdadero:** medido, **cero coocurrencias** de `reciproc` con `super class`, `cell class`, `cell categor`, `sensory neuron`, `motor neuron` o `descending` en los 131.712 caracteres del cuerpo de Lin. **Su eje es ANATÓMICO (78 neuropilos, dónde caen las sinapsis). El de la Table 7 es FUNCIONAL Y DIRIGIDO (sensorial→motor, óptico→motor, intra-motor).** Ejes ortogonales, preguntas distintas. **La novedad no es «descomponer»: es sobre qué eje.**
...
**Control negativo `ZZQQXX` = 0 en los tres; positivo alto en los tres.** El guard puede dar rojo (modo de falla 6 evitado).

Sus 2 menciones son prosa. **Una importa y hay que citarla:** *«The CNS networks with a high influence on effectors are directly linked in a **nearly all-to-all pattern of reciprocal connectivity** (Fig. 6c,d)»*. Reciprocidad **entre módulos**, cualitativa, sin ratio y sin null. **No es prior art del desglose, pero es adyacente.**
...
La última frase del abstract de Lin: *«These data products… **should serve as a foundation for models and experiments exploring the relationship between neural activity and anatomical structure**»*. **Eso es lo que hace el Paper 1.** Lin: estructura. Paper 1: primera película dinámica. BANC: institucionalización del baseline lineal a escala grande. **Y BANC no te tomó: su preprint es de jul-2025, tu preprint de mar-2026. Es convergencia metodológica.**
...
| **🟢 Acceso motor a 2 SALTOS · el rescate de la Propiedad 2** | **visual alcanza 95.160 nodos y toca 15 de 110 motoras · mechano alcanza 68.471 y toca 110/110 · gustatory 26.535 y 107 · olfactory 22.940 y 23.** La que llega **más lejos** es la que toca **menos** | resp 057, medido |
...
| **«el erratum corrige un 1.559× que aparece 9 veces»** (borrador) | **🔴 RETIRADO (resp 046):** el 1.559 **no existe en el PDF**. **Un erratum que corrige un claim que el paper no hizo es él mismo un error** ← **y esta misma falla se repitió apuntando a Nature, ver arriba** |
...
| **«el erratum está listo / no está listo / no existe en git»** | **CERRADO: `docs/ERRATUM.md` existe, 9 ítems, cero placeholders, y el claim falso retirado.** Falta **subirlo a Zenodo** |
...
| «visual es la vía con menos acceso motor» | **no sobrevive a 1 salto**: es **olfactory** (doc `5937`). **Pero a 2 saltos visual es la que menos motoras toca (15 vs 23)** |
...
8. **🔴 Los valores de la Fig. 5c y la ED Fig. 6c de Lin NO se leyeron fila por fila.** Sé qué miden por el pie de figura y el texto. **Si algún neuropilo corresponde funcionalmente a un circuito de la Table 7, «ortogonal» es demasiado generoso.** Y **no busqué si publican la tabla numérica como Supplementary Data descargable**: si existe, se cruza directo.
9. **De Lin leí el cuerpo (introducción a reciprocidad y neuropilos), no su Discussion ni sus Methods.** La **Table 2 sí está leída fila por fila** (los ×858/×43,8/×45,9/×7,22). **No leí Dorkenwald et al.**
10. **NPC vs CP no se compararon midiendo.**
11. **🔴 Los conteos a 2 saltos NO tienen null.** Que visual toque 15/110 y mechano 110/110 es contraste crudo. **Sin null preservando grado es una observación, no un resultado publicable.** Y **ignora signo y peso**: cuenta existencia de camino, no influencia.
12. **🔴 Discrepancia sin resolver: 105 vs 110 motoras de cabeza.** La resp 053b usó un filtro `brain_motor_neuron` (105, con CV 19 y ON 6); la resp 057 usó `super_class == 'motor'` (110, con CV 20 y ON 10). **No está establecido cuál corresponde a la población del paper.** Los ceros de 1 salto (0, 0) no cambian, **pero el denominador de cualquier fracción está sin fijar**, y «15 de 110» es una fracción.
13. **El test de solapamiento de neuropilo NO se pudo correr:** `annotations.tsv` **no tiene columna de neuropilo**. La explicación anatómica del cero está **apoyada por literatura y por los destinos a 1 salto, no medida sobre neuropilos**.
14. **No barrí exhaustivamente si el cero ya está publicado.** Cinco búsquedas. Lo más cercano: **Miroschnikow et al. 2018** (eLife, citado por BANC) muestra sensorial→motor monosináptico **sí existe** en la **larva**; y los papers de grooming de cabeza (Hampel 2015, Eichler 2024, Calle-Schuler 2026, Nat Commun abr-2026) trabajan con circuitos multicapa **como supuesto, no como cero medido**. **Apoyado, no establecido.**
15. **Los tres notebooks con overflow se declaran descendientes** de un «pipeline original» que **no está en el corpus**. **Apoyado** por reproducir 8 de 9 ratios, **no establecido**.
16. **Un solo patrón de overflow barrido** (`N*(N-1)`). No barrí `N**2`.
...
18. **Los dos `.mjs` del release no corren en un clon ajeno:** rutas absolutas `/workspace/...`. **Declarado en README y METHODS, no parcheado.** Mi test de «clon limpio» dio un **falso verde** (resp 047).
19. **Los dos JSON grandes van por md5**, no commiteados: `nulls40.json` (191.443 B, `38bf1fcadaf37a3b125f83d22b6f4d8e`) y `dualbrain_bench.json` (31.527 B, `1025d60b4e9521d7e4a21ed282935049`).
20. **Los 6 `.py` de deuda siguen fuera de git.** Manifiesto en `MANIFIESTO-KAGGLE.md`.
21. **El C99 embebido está medido a medias:** 1.336 B de `.text` en target, **sin `.elf`, sin RAM en target**, no corrió en hardware.
22. **No corrí el review automático** sobre los archivos nuevos. **K-02: deuda declarada.**
23. **El `README.md` y `docs/METHODS.md` NO tienen la evidencia nueva** de las resp 053b y 057. Y sigue pendiente la reclasificación del `temporal RDI`.

---

## 6. Decisiones esperando a Abraham

1. **🔴 SUBIR LA v2 A ZENODO antes del 30-ago.** El texto está en **`docs/ERRATUM.md`**, 9 ítems, cero placeholders, **con el claim falso sobre Lin retirado y Bates citado**. **Falta solo tu acción.**
...
4. **Bajar el 36× y el «massive reciprocity» del abstract**, y poner en su lugar el **eje funcional** de la Table 7 (no «el desglose», que tiene prior art anatómico). Texto listo en **`docs/PIVOTE-RECIPROCIDAD.md`**.
5. **Citar a Lin, el NPC model y Bates et al. como prior art** en la v2. Ya está en el erratum §§3, 5, 8 y en Outstanding verification 7-9.
6. **Agregar el párrafo de convergencia** attractor/repeller. Lo más barato del plan.
7. **¿Adoptar el umbral de 5 sinapsis?** **Los TRES papers de referencia lo usan** (Dorkenwald, Lin, Bates). El Paper 1 es el único que no. **Re-correr o justificar.**
...
16. **🆕 ¿Corro el null de grado sobre los conteos a 2 saltos?** Sin él, el rescate de la Propiedad 2 no se publica. Y **fijar el filtro de la población motora** (105 vs 110) es prerrequisito.

---

## 7. Estado de git y del entorno, medido

**Ya en el repo:** `docs/ERRATUM.md` (**9 ítems, corregido el 24-ago 23:15**) · `docs/PIVOTE-RECIPROCIDAD.md` (**corregido**) · `README.md` · `LICENSE` · `docs/METHODS.md` · `src/` con 8 archivos · `results/` con 5 logs · `docs/agents/` con los cuatro contextos, el índice, el manifiesto de Kaggle, la evidencia y **59 respuestas**.
...
**Entorno:** el container **no es efímero** (uptime 3,2 días medido), `git` no está instalado y todo pasa por la integración de GitHub, `nexus.db` **no existe**. Python 3.12.14, Node 24.18.0, R 4.5.3. **`pypdf` no venía instalado y se instala con `pip` (hay red).** **Los Docs son ENUMERABLES:** IDs de página secuenciales **paso 20**, prefijo `2kza6fw5-`. Límite medido: **5 Docs completos no entran en una ventana**. **El resto se re-mide:** `CONTEXTO-ENTORNO.md` §13.

**Datos:** `/workspace/connectivity.parquet` (100.804.642 B, 15.091.983 filas, md5 `3d802fd542b5d18570ba1ba0bb0abed9`) y `/workspace/annotations.tsv` (31.718.505 B, md5 `719904abad876c68ace1b5690c9b9b63`). **Columnas de anotación: 31, y NINGUNA es neuropilo.**
...
**2 · Un null cuyos invariantes incluyen la cantidad medida no es un control, es un espejo.** **Si `sd(null) == 0`, reportar NO TESTEABLE**, no `1,000×`.
...
**5 · Comparar dos cantidades medidas con criterios distintos. CINCO veces.** El `5177`, el `5117`, el borrador del erratum, el `1,18× sobre LSTM` contra el `4× en contra`, **y la Table 7 sin umbral conviviendo con el 13,98% con umbral en el mismo párrafo del pivote** (esta vez declarado, no cometido).

**6 · Un test que no puede dar rojo no es un test.** El `$?` reportando `exit=0` sobre una compilación fallida, un `grep -c` con falso cero por la coma de miles, y un «clon limpio» que leía rutas absolutas. **Antídoto aplicado el 24-ago: todo conteo de términos va con control positivo Y negativo (`ZZQQXX`).**
...
**10 · 🆕 Un claim de novedad se verifica contra las FIGURAS del trabajo previo, no contra su abstract.** El erratum llegó a decir que Lin *«reporta solo una cifra global»* porque leí su abstract, su Table 2 y su sección de reciprocidad, **y no su lista de figuras**, donde la Fig. 5c y la ED Fig. 6c hacen exactamente lo que yo estaba reclamando como propio. **Un paper no anuncia en el abstract todo lo que midió.** Y esto casi se va a un DOI con nombre y apellido de otros autores adentro.

**11 · 🆕 Verificar el sujeto no alcanza: hay que verificar la ESCALA a la que la explicación rival es cierta.** Refuté «localidad anatómica» a escala de parte del cuerpo y canté victoria, cuando a escala de **neuropilo** la localidad **explica** mi cero, y es literatura de hace 25 años. **Una explicación rival puede ser falsa a una granularidad y verdadera a otra: hay que decir a cuál.**
