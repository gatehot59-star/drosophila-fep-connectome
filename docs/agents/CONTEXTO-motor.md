...
**La frase publicable, si el brazo `D` confirma:** *al mismo presupuesto de parámetros, la estructura sin entrenar aporta más que el entrenamiento sin la estructura.* No reemplaza al entrenamiento (A le gana a B y C siempre), pero lo domina en magnitud.
...
3. **El brazo `D`** (`DualNoGate` en la misma corrida, `h_r=26 h_m=8`): **~25 min**, y sin él el hallazgo de §3 es cross-run.
...
6. Log de `titan-paper-dualbrain` (complete 00:06Z) y `notebookceb82767da` (928 KB): sin leer.
