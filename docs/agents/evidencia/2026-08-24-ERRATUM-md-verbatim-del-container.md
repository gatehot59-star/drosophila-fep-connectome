# EVIDENCIA · `ERRATUM.md` verbatim, rescatado del container

> **⚠️ ESTO NO ES EL ERRATUM CANÓNICO. Es una copia de evidencia.**
>
> **Origen:** `/workspace/repo/docs/ERRATUM.md` en el container de Abraham.
> **md5:** `2ae28606c28c140dc76cd3b8e6b3ab85` · **125 líneas, 6.862 B.**
> **Leído y copiado:** 2026-08-24 14:00 (America/Buenos_Aires), resp 039.
>
> **Por qué se rescata:** este archivo es el entregable del **umbral #1 del plan de 10 semanas** (subir el erratum a Zenodo **antes del 30-ago**) y **no existía en git**: `docs/` en `main` contenía únicamente el subdirectorio `agents/`. Vivía en un solo lugar, sin versionar, dentro de `/workspace/repo/`, que es el release de 11 archivos que nunca se commiteó. Un container reciclado y se perdían 125 líneas.
>
> **Por qué NO se commiteó como `docs/ERRATUM.md`:** hay **tres textos de erratum incompatibles** y elegir cuál es el bueno es decisión de Abraham, no mía. Ponerlo en la raíz de `docs/` lo volvería canónico por ubicación.
>
> | Versión | Dónde | Ítems |
> |---|---|---|
> | **ésta** | container, `/workspace/repo/docs/`, sin commitear | **5** más «What is not corrected» |
> | E1-E8 | Doc `2kza6fw5-5157` | **8**, con *«tres corchetes sin rellenar a propósito»* |
> | 7 puntos | Doc `2kza6fw5-6117`, con un **E7** sobre el desfasaje | **7** |
>
> **Verificado sobre ESTE archivo, con grep:** cero ocurrencias de `Table 7`, cero de `Ratio`, cero de `XXXX`/`[TBD`/`[DOI`/`TODO`, cero de `desfas`/`off-by`. O sea que los tres bloqueos que los contextos le atribuían **no son de este texto**.
>
> **El defecto real que SÍ tiene (resp 039 §3.3):** su ítem 1 publica la reciprocidad como **338,8×**, que es el ratio contra densidad uniforme, el null más débil. El número defendible medido contra 40 nulls CP es **20,59×** (doc `6057`, 0/40). Su ítem 2, en cambio, **sí** usa el null que preserva grado (283×, olfactory como más depletado) y está al día.
...
## Los otros 10 archivos del release sin commitear, para que no se pierdan de vista

```
repo/LICENSE
repo/README.md
repo/docs/METHODS.md              93 lineas, md5 0c2f9bf2d4b9f6bcaaf6cbaad1bf08b9
repo/src/analyze_nulls40.mjs
repo/src/nulls40_structural.py
repo/src/routing_hierarchy.mjs
repo/results/dualbrain_bench.json
repo/results/dualbrain_bench.log
repo/results/nulls40.json
repo/results/nulls40.log
```

**Ninguno de los diez fue leído en la resp 039.** El ítem 4 del erratum cambia la licencia y el ítem 5 cita `METHODS.md`: los dos archivos están en esta lista y siguen sin versionar.
